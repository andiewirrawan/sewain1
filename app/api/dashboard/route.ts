...
export async function GET(request: Request) {
  try {
    ...
    const { data: kontrak } = await supabase.from('kontrak_sewa').select('id_kontrak, id_penyewa, tanggal_jatuh_tempo, status_kontrak');
    ...
    // 3. Data Pembayaran
    const { data: pembayaran } = await supabase.from('pembayaran').select('id_pembayaran, id_kontrak, nominal, status_pembayaran, periode');
